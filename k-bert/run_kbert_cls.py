    ...
    for line_id, line in enumerate(sentences):
        ...
        try:
            if len(line) == 2:
                ...
                text = CLS_TOKEN + line[columns["content"]]
...
def main():
    ...
    labels_set = set()
    columns = {}
    with open(args.train_path, mode="r") as f:
    ...
    # Datset loader.
    def normal_batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vms):
    ...
    def batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vms):
        ...
        for l in range(len(labels)):
            label_list[labels[l]].append(l)
        label_len = []
        for l in range(len(label_list)):
            label_len.append(len(label_list[l]))
        max_len = max(label_len)
        mini_batch_size = int(batch_size / 6)
        for i in range(max_len // mini_batch_size):
            zero = []
            head_zero = i * mini_batch_size % label_len[0]
            tail_zero = (i + 1) * mini_batch_size % label_len[0]
            if head_zero > tail_zero:
                zero = label_list[0][:tail_zero] + label_list[0][head_zero:]
            else:
                zero = label_list[0][head_zero:tail_zero]
            one = []
            head_one = i * mini_batch_size % label_len[1]
            tail_one = (i + 1) * mini_batch_size % label_len[1]
            if head_one > tail_one:
                one = label_list[1][:tail_one] + label_list[1][head_one:]
            else:
                one = label_list[1][head_one:tail_one]
            two = []
            head_two = i * mini_batch_size % label_len[2]
            tail_two = (i + 1) * mini_batch_size % label_len[2]
            if head_two > tail_two:
                two = label_list[2][:tail_two] + label_list[2][head_two:]
            else:
                two = label_list[2][head_two:tail_two]
            three = []
            head_three = i * mini_batch_size % label_len[3]
            tail_three = (i + 1) * mini_batch_size % label_len[3]
            if head_three > tail_three:
                three = label_list[3][:tail_three] + label_list[3][head_three:]
            else:
                three = label_list[3][head_three:tail_three]
            four = []
            head_four = i * mini_batch_size % label_len[4]
            tail_four = (i + 1) * mini_batch_size % label_len[4]
            if head_four > tail_four:
                four = label_list[4][:tail_four] + label_list[4][head_four:]
            else:
                four = label_list[4][head_four:tail_four]
            five = []
            head_five = i * mini_batch_size % label_len[5]
            tail_five = (i + 1) * mini_batch_size % label_len[5]
            if head_five > tail_five:
                five = label_list[5][:tail_five] + label_list[5][head_five:]
            else:
                five = label_list[5][head_five:tail_five]

            batch = zero + one + two + three + four + five
            # print("这批数据的id为： ", batch)
            input_ids_batch = input_ids[batch, :]
            label_ids_batch = label_ids[batch]
            mask_ids_batch = mask_ids[batch, :]
            pos_ids_batch = pos_ids[batch, :]
            vms_batch = []
            for b in batch:
                vms_batch.append(vms[b])
            yield input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch
    ...
    def evaluate(args, is_test, metrics='Acc'):
        ...
        if not args.mean_reciprocal_rank:
            for i, (input_ids_batch, label_ids_batch,  mask_ids_batch, pos_ids_batch, vms_batch) in \
                    enumerate(normal_batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vms)):
